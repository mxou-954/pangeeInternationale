...
const EquipmentDisplayPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await getAllEquipements({ signal: ac.signal });
        if (ac.signal.aborted) return;
        console.log('Données reçues:', data);
        setEquipmentItems(data as any[]);
      } catch (err) {
        if (!ac.signal.aborted) console.error('Erreur fetch équipements:', err);
      }
    })();
  ...
  const handleDelete = async (itemId: string) => {
    try {
      ...
      setEquipmentItems((prev) => prev.filter((f) => f.id !== itemId));
    } catch (err) {
  ...
  const handleSave = async (equipementId: string) => {
    ...
    try {
      const data = await quickEditEquipement(String(equipementId), toSend);
      console.log('Équipement mis à jour', data);

      // Option: MAJ locale
      // setEquipmentItems(prev => prev.map(it => it.id === data.id ? data : it));
      // setMAJ(false);
    } catch (err) {
